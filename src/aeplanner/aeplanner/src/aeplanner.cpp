...
Eigen::Vector4d AEPlanner::sampleNewPoint()
{
  ...
  do
  {
    ...
    double search_location_z = point[2];
...
void AEPlanner::wifiDistCallback(const geometry_msgs::PoseStamped& msg)
{
  wifi_dist_state_[0]  = msg.pose.position.x;
  wifi_dist_state_[1]  = msg.pose.position.y;
