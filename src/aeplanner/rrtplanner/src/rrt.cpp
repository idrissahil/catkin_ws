...
Rrt::Rrt(const ros::NodeHandle& nh)
  : nh_(nh)
  , frame_id_("map")
  , path_pub_(nh_.advertise<visualization_msgs::Marker>("rrt_path1", 1000))
  ...
  , as_(nh_, "rrt", boost::bind(&Rrt::execute, this, _1), false)
{
  ...
  min_nodes_ = 100;
  ROS_INFO_STREAM("testing  " << 2);
  double test2=2;
  printf ("Testing printf %f.\n", test2 );    
...
bool Rrt::collisionLine(Eigen::Vector3d p1, Eigen::Vector3d p2, double r)
{
  ...
  double Amp=1;
  //double curr_location_x = 4;
  //double curr_location_y = 4;

  //double curr_location_z = 4;

  double search_location_x = p2[0];
  double search_location_y = p2[1];

  double search_location_z = p2[2];
  ...
  ROS_INFO_STREAM("wifi signal info stream " << wifi_sig);

  //if (wifi_sig<0.9)
  //{
  //      ROS_INFO_STREAM("wifi signal too low on searchpoint " << wifi_sig);
  //      return true;
